...
const commandUsage: CommandUsage = {
  long: `Reopen a closed Backlog pull request.

Sets the pull request status back to Open. Optionally add a comment.`,

  examples: [
    { description: "Reopen a pull request", command: "bee pr reopen 42 -p PROJECT -R repo" },
    {
      description: "Reopen with a comment",
      command: 'bee pr reopen 42 -p PROJECT -R repo -c "Reopening for further review"',
    },
  ],
  ...
  defineCommand({
    ...
    async run({ args }) {
      ...
      const pullRequest = await client.patchPullRequest(args.project, args.repo, prNumber, {
        statusId: PrStatusId.Open,
        comment: args.comment ? [args.comment] : undefined,
        notifiedUserId,
      } as never);

      outputResult(pullRequest, args, (data) => {
        consola.success(`Reopened pull request #${data.number}: ${data.summary}`);
      });
    },
  }),
  commandUsage,
