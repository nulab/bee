...
`-y, --yes` フラグはすべてのコマンドで利用できるグローバルオプションです。削除などの破壊的操作で表示される確認プロンプトをスキップします。AI エージェントは対話的な確認に応答できないため、破壊的操作には常に `--yes` を付けてください。

```sh
bee issue delete PROJECT-123 --yes
```

確認プロンプトのないコマンドに `--yes` を付けても無視されるため、エージェントが一律に付与しても安全です。
