...
2. **Skill を登録する**
   ...
   Backlog ではプロジェクトごとに Markdown か Backlog 記法を選択できます。Markdown のプロジェクトで誤って適用されないよう、`backlog-notation` には `disable-model-invocation: true` を指定してあり、エージェントが自分の判断で読み込むことはありません。使いたいときはスラッシュコマンドで明示的に呼び出してください。
