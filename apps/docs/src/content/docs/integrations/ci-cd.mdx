...
title: CI/CD での利用
description: |-
  GitHub Actions・GitLab CI・Bitbucket Pipelines・CircleCI・Jenkins から bee CLI で Backlog 課題を自動操作する方法。
---
...
`-y, --yes` フラグは**すべてのコマンドで利用できるグローバルオプション**です。確認プロンプトをスキップするため、非対話環境では常に付けることを推奨します。

```sh
# 確認なしで課題を削除
npx @nulab/bee issue delete PROJECT-123 --yes
```

確認プロンプトのないコマンド（`list`、`view` など）に `--yes` を付けても無視されるため、スクリプトで一律に付けても安全です。
...
<LinkCard title="PR と課題を連動させる" href="/bee/recipes/pr-lifecycle-sync/" description="PR の作成・編集・マージに連動して課題のステータス更新やクローズを自動化" />
<LinkCard title="リリース時に課題へ通知" href="/bee/recipes/release-notify-issues/" description="リリースタグの作成時に関連課題へコメントし、リリースノートを Backlog ドキュメントに投稿" />
