import type { MockConsola } from "@repo/test-utils";
...
vi.mock("consola", () => {
  ...
  return { default: mockConsola };
});

// eslint-disable-next-line import/first -- vi.mock must be hoisted before imports
...
describe("promptRequired", () => {
  ...
  it("既存の値がない場合はプロンプトを表示する", async () => {
    vi.mocked(mockConsola.prompt).mockResolvedValue("user-input" as never);

    const result = await promptRequired("Label:");
    expect(mockConsola.prompt).toHaveBeenCalledWith("Label:", { type: "text" });
    expect(result).toBe("user-input");
  });

  it("既存の値が undefined の場合はプロンプトを表示する", async () => {
    vi.mocked(mockConsola.prompt).mockResolvedValue("prompted" as never);
  ...
  it("プロンプトで空文字が入力された場合は process.exit(1) を呼ぶ", async () => {
    vi.mocked(mockConsola.prompt).mockResolvedValue("" as never);
    const mockExit = spyOnProcessExit();

    await promptRequired("Label:");

    expect(mockConsola.error).toHaveBeenCalledWith("Label is required.");
  ...
  it("options が consola.prompt に渡される", async () => {
    ...
    expect(mockConsola.prompt).toHaveBeenCalledWith("Label:", {
      type: "text",
  ...
  it("ラベル末尾のコロンを除去してエラーメッセージを生成する", async () => {
    ...
    expect(mockConsola.error).toHaveBeenCalledWith("Project key is required.");
    mockExit.mockRestore();
  });
...
describe("confirmOrExit", () => {
  ...
  it("ユーザーが確認した場合、true を返す", async () => {
    vi.mocked(mockConsola.prompt).mockResolvedValue(true as never);

    const result = await confirmOrExit("Are you sure?");
    expect(mockConsola.prompt).toHaveBeenCalledWith("Are you sure?", { type: "confirm" });
    expect(result).toBeTruthy();
  });

  it("ユーザーがキャンセルした場合、false を返し Cancelled. を表示する", async () => {
    vi.mocked(mockConsola.prompt).mockResolvedValue(false as never);

    const result = await confirmOrExit("Are you sure?");
    expect(mockConsola.prompt).toHaveBeenCalledWith("Are you sure?", { type: "confirm" });
    expect(mockConsola.info).toHaveBeenCalledWith("Cancelled.");
